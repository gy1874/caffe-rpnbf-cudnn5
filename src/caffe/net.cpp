...
namespace caffe {
	...
	void Net<Dtype>::Init(const NetParameter& in_param,
		...
		for (int input_id = 0; input_id < param.input_size(); ++input_id) {
			...
			AppendTop(param, layer_id, input_id, &available_blobs, &blob_name_to_idx, memory_share_net);
		}
		...
		for (int layer_id = 0; layer_id < param.layers_size(); ++layer_id) {
			...
			for (int top_id = 0; top_id < num_top; ++top_id) {
				AppendTop(param, layer_id, top_id, &available_blobs, &blob_name_to_idx, memory_share_net);
			}
			...
			if (layer->AutoTopBlobs()) {
				...
				for (; num_top < needed_num_top; ++num_top) {
					...
					// to other layers.
					AppendTop(param, layer_id, num_top, NULL, NULL, memory_share_net);
				}
			}
	...
	void Net<Dtype>::AppendTop(const NetParameter& param, const int layer_id,
		...
		map<string, int>* blob_name_to_idx, Net<Dtype>* memory_share_net) {
